...
run() {
  ERRF=$(mktemp)
  OUT=$(env -i PATH="$PATH" "$@" $SH -c 'set -eu; fail() { echo "[start] FATAL: $*" >&2; exit 1; }; . "$0/docker/documents.sh"; documents_settings; echo "$DOC_ENABLED $DOC_MAX_FILE_BYTES $DOC_USER_QUOTA_BYTES $DOC_MAX_PAGES"' "$REPO" 2>"$ERRF"); RC=$?
  ERR=$(cat "$ERRF")
}

echo "== defaults"
run
check "defaults: on, 200 MB file, 1 GB per user, 1500 pages" "$([ "$OUT" = "true 209715200 1073741824 1500" ] && echo 1)" "rc=$RC out=$OUT $ERR"
...
check "custom values are converted to bytes" "$([ "$OUT" = "true 52428800 524288000 800" ] && echo 1)" "$OUT $ERR"
run DOCUMENT_MAX_FILE_MB= DOCUMENT_USER_QUOTA_MB= DOCUMENT_MAX_PAGES=
check "empty values (compose pass-through) mean the defaults" "$([ "$OUT" = "true 209715200 1073741824 1500" ] && echo 1)" "$OUT $ERR"
run DOCUMENT_MAX_FILE_MB=08 DOCUMENT_USER_QUOTA_MB=0100
check "leading zeros are decimal, not octal" "$([ "$OUT" = "true 8388608 104857600 1500" ] && echo 1)" "rc=$RC $OUT $ERR"
