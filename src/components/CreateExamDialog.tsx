...
export function CreateExamDialog({ courses, open, onOpenChange, onSuccess, timezone, defaultDate }: CreateExamDialogProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (open && defaultDate) setFormData((prev) => ({ ...prev, examDate: defaultDate }));
  }, [open, defaultDate]);
