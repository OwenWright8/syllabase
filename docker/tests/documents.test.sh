...
run() {
  ERRF=$(mktemp)
  OUT=$(env -i PATH="$PATH" "$@" $SH -c 'set -eu; fail() { echo "[start] FATAL: $*" >&2; exit 1; }; . "$0/docker/documents.sh"; documents_settings; echo "$DOC_ENABLED $DOC_MAX_FILE_BYTES $DOC_USER_QUOTA_BYTES $DOC_MAX_PAGES $DOC_MAX_COUNT"' "$REPO" 2>"$ERRF"); RC=$?
  ERR=$(cat "$ERRF")
}

echo "== defaults"
run
check "defaults: on, 200 MB file, 1 GB per user, 1500 pages, 200 documents" "$([ "$OUT" = "true 209715200 1073741824 1500 200" ] && echo 1)" "rc=$RC out=$OUT $ERR"
...
echo "== limits"
run DOCUMENT_MAX_FILE_MB=50 DOCUMENT_USER_QUOTA_MB=500 DOCUMENT_MAX_PAGES=800
check "custom values are converted to bytes" "$([ "$OUT" = "true 52428800 524288000 800 200" ] && echo 1)" "$OUT $ERR"
run DOCUMENT_MAX_FILE_MB= DOCUMENT_USER_QUOTA_MB= DOCUMENT_MAX_PAGES=
check "empty values (compose pass-through) mean the defaults" "$([ "$OUT" = "true 209715200 1073741824 1500 200" ] && echo 1)" "$OUT $ERR"
run DOCUMENT_MAX_FILE_MB=08 DOCUMENT_USER_QUOTA_MB=0100
check "leading zeros are decimal, not octal" "$([ "$OUT" = "true 8388608 104857600 1500 200" ] && echo 1)" "rc=$RC $OUT $ERR"
...
run DOCUMENT_MAX_PAGES=0;       check "DOCUMENT_MAX_PAGES=0 is rejected" "$([ $RC -ne 0 ] && [[ "$ERR" == *"DOCUMENT_MAX_PAGES"* ]] && echo 1)" "$ERR"
...
echo "== unlimited"
run DOCUMENT_MAX_FILE_MB=unlimited DOCUMENT_USER_QUOTA_MB=unlimited DOCUMENT_MAX_PAGES=unlimited DOCUMENT_MAX_COUNT=unlimited
check "unlimited becomes numbers far beyond anything real (1 PB, a million pages/documents)" "$([ "$OUT" = "true 1000000000000000 1000000000000000 1000000 1000000" ] && echo 1)" "rc=$RC $OUT $ERR"
run DOCUMENT_MAX_FILE_MB=UNLIMITED DOCUMENT_USER_QUOTA_MB=Unlimited
check "the word is case-insensitive" "$([ "$OUT" = "true 1000000000000000 1000000000000000 1500 200" ] && echo 1)" "rc=$RC $OUT $ERR"
run DOCUMENT_USER_QUOTA_MB=unlimited
check "an unlimited quota with the default file limit is fine" "$([ "$OUT" = "true 209715200 1000000000000000 1500 200" ] && echo 1)" "rc=$RC $OUT $ERR"
run DOCUMENT_MAX_FILE_MB=unlimited DOCUMENT_USER_QUOTA_MB=500
check "an unlimited file size under a finite quota is fine (the quota still bounds it)" "$([ $RC -eq 0 ] && [ "$OUT" = "true 1000000000000000 524288000 1500 200" ] && echo 1)" "rc=$RC $OUT $ERR"
